...
details are in \textbf{\texttt{README-AE.md}}.

\subsection{Description \& Requirements}

The main Rust crate contains \textbf{Isotaxis}, \textbf{Pompe}, \textbf{clients}, and \textbf{adversarial nodes};
...
instances.
Each region must have the security-group rules documented in README-AE.md;
account-specific identifiers are not included in the artifact.
First inspect the plan and dry-run:
...
account with existing instances and their SSH private key, \textit{execute:}
...
These steps generate configuration, initialize and deploy the hosts, pull the
pinned Docker Hub image, run the experiment, stop the containers, and collect
logs.  Repeat the documented batch/load sweep for both protocols and record
the plan and configuration.  \textit{Results:} \path{ec2/ec2-logs} contains
client latency and committed throughput for reconstructing the Fig.~6 panel.
