...
pub struct PompeNetwork {
    ...
    queue_saturation_drops: Arc<AtomicU64>,
}
...
impl PompeNetwork {
    pub fn new(node_id: usize, peer_node_ids: Vec<usize>) -> Self {
        ...
        let network = Self {
            ...
            queue_saturation_drops: Arc::new(AtomicU64::new(0)),
        };
    ...
    fn enqueue_to_handle(
        ...
        match handle.sender.try_send(msg) {
            ...
            Err(TrySendError::Full(_msg)) => {
                let drops = self.queue_saturation_drops.fetch_add(1, Ordering::Relaxed) + 1;
                if drops == 1 || drops % 10_000 == 0 {
                    warn!(
                        "[pompe-network] node {} outbound queues saturated; {} messages dropped so far",
                        self.node_id, drops
                    );
                }
                if attack_only_mode() {
    ...
    pub fn queue_saturation_drops(&self) -> u64 {
        self.queue_saturation_drops.load(Ordering::Relaxed)
    }
}
...
impl Clone for PompeNetwork {
    fn clone(&self) -> Self {
        Self {
            ...
            queue_saturation_drops: Arc::clone(&self.queue_saturation_drops),
        }
    }
}
