...
const DEFAULT_MAX_SEQ_DISTANCE: u64 = 200;
const ENV_MAX_SEQ_DISTANCE: &str = "SMROL_MAX_SEQ_DISTANCE";
...
pub struct TransactionSequencing {
    ...
    /// Maximum deferred requests retained for each authenticated sender.
    max_pending_seq_requests: usize,
    max_seq_distance: u64,
    ...
    pending_seq_request_counts: DashMap<usize, usize>,
    highest_contiguous_ready_slot_by_sender: DashMap<usize, u64>,
    rejected_seq_requests: AtomicU64,
...
impl TransactionSequencing {
    pub fn new(
        ...
        let max_seq_distance = std::env::var(ENV_MAX_SEQ_DISTANCE)
            .ok()
            .and_then(|raw| raw.trim().parse::<u64>().ok())
            .filter(|value| *value > 0)
            .unwrap_or(DEFAULT_MAX_SEQ_DISTANCE);
        ...
        Self {
            ...
            pending_seq_request_counts: DashMap::new(),
            highest_contiguous_ready_slot_by_sender: DashMap::new(),
            rejected_seq_requests: AtomicU64::new(0),
    ...
    fn enqueue_pending_seq_request(
    ...
    ) -> bool {
        {
            let mut sender_count = self
                .pending_seq_request_counts
                .entry(leader_id)
                .or_insert(0);
            if *sender_count >= self.max_pending_seq_requests {
                drop(sender_count);
                self.record_rejected_seq_request(
                    "per-sender pending limit",
                    leader_id,
                    pending_req.sequence_number,
                );
                return false;
            }
            *sender_count += 1;
        ...
        let pending_size = self
            ...
            .map(|count| *count)
            .unwrap_or(0);
        metrics::gauge!("smrol.pending_seq_requests", "sender" => leader_id.to_string())
            .set(pending_size as f64);
        debug!(
            "[Sequencing] Node {} pending SeqRequest waitlist size for sender {}: {}",
            self.process_id, leader_id, pending_size
        );
        true
    }

    fn record_rejected_seq_request(&self, reason: &str, sender_id: usize, sequence_number: u64) {
        metrics::counter!("smrol.rejected_seq_requests").increment(1);
        let rejected = self.rejected_seq_requests.fetch_add(1, Ordering::Relaxed) + 1;
        if rejected == 1 || rejected % 10_000 == 0 {
            warn!(
                "[Sequencing] Node {} rejected SeqRequest from sender {} at sequence {}: {} (total rejected={})",
                self.process_id, sender_id, sequence_number, reason, rejected
            );
        }
    }
    ...
    async fn process_seq_request_message(
        ...
        let highest_ready_slot = self
            .highest_contiguous_ready_slot_by_sender
            .get(&sender_id)
            .map(|slot| *slot)
            .unwrap_or(0);
        let expected_sequence = highest_ready_slot.saturating_add(1);
        let sequence_distance = if sequence_number >= expected_sequence {
            sequence_number - expected_sequence
        } else {
            expected_sequence - sequence_number
        };
        if sequence_distance > self.max_seq_distance {
            self.record_rejected_seq_request(
                "sequence outside accepted window",
                sender_id,
                sequence_number,
            );
            return Ok(());
        ...
        if sequence_number > 1 && !self.pnfifo.has_output(sender_id, sequence_number - 1) {
            ...
            if !self.enqueue_pending_seq_request(sender_id, wait_slot, pending_req) {
                return Ok(());
            }
            return Ok(());
        }
    ...
    fn on_slot_ready(&self, leader_id: usize, slot: u64) {
        {
            let mut highest = self
                ...
                .entry(leader_id)
                .or_insert(0);
            if slot == (*highest).saturating_add(1) {
                *highest = slot;
                while self
                    .pnfifo
                    .has_output(leader_id, (*highest).saturating_add(1))
                {
        ...
        if let Some((_, pending)) = self.pending_seq_requests.remove(&(leader_id, slot)) {
            ...
            let remaining =
                if let Some(mut count) = self.pending_seq_request_counts.get_mut(&leader_id) {
                    *count = (*count).saturating_sub(resumed);
                    *count
                } else {
                    0
                };
            ...
            debug!(
                ...
                self.process_id, resumed, leader_id, slot
            );
