...
pub struct LatencyTracker {
    ...
    log_individual_latency: bool,
    measurement_started_at: Option<Instant>,
    warmup_duration: Duration,
    excluded_warmup_transactions: usize,
}
...
const INITIAL_CONNECT_MAX_ATTEMPTS: usize = 10;
const INITIAL_CONNECT_RETRY_DELAY: Duration = Duration::from_millis(500);
const DEFAULT_LATENCY_SUMMARY_INTERVAL: usize = 500;
...
impl LatencyTracker {
    pub fn new(log_individual_latency: bool, warmup_secs: u64) -> Self {
        Self {
            ...
            log_individual_latency,
            measurement_started_at: None,
            warmup_duration: Duration::from_secs(warmup_secs),
            excluded_warmup_transactions: 0,
        }
    }

    pub fn start_measurement(&mut self) {
        self.measurement_started_at = Some(Instant::now());
    }

    pub fn record_send_time(&mut self, tx: &TestTransaction) {
        if self
            .measurement_started_at
            .is_some_and(|started_at| started_at.elapsed() < self.warmup_duration)
        {
            self.excluded_warmup_transactions += 1;
            return;
        }
    ...
    pub fn handle_ordering_response(&mut self, tx_ids: Vec<u64>) {
        for tx_id in tx_ids {
            ...
            if let Some(send_time) = self.send_timestamps.get(&base_id) {
                ...
                if self.log_individual_latency {
    ...
    pub fn print_consensus_stats(&self) {
        if self.consensus_latencies.is_empty() {
            info!(
                "[consensus] no latency samples; committed transaction IDs did not match measured sends"
            );
            return;
        }
    ...
    pub fn print_comprehensive_stats(&self) {
        ...
        info!(
            "[latency] warm-up: consensus-only, no client transactions, duration={} s",
            self.warmup_duration.as_secs()
        );
        info!(
            "[latency] excluded warm-up transactions={}",
            self.excluded_warmup_transactions
        );
...
impl StatsReporter {
    ...
    pub fn should_print_stats(&mut self) -> bool {
        if self.consensus_samples < self.next_report_at {
            return false;
        }

        while self.next_report_at <= self.consensus_samples {
            self.next_report_at += self.report_interval;
        }
        true
    }
}
...
impl ClientNode {
            ...
            for attempt in 1..=INITIAL_CONNECT_MAX_ATTEMPTS {
                ...
                .await
                {
                    Ok(conn) => {
                        ...
                        info!(
                            "[client] connected to node {} on attempt {}/{}",
                            node_id, attempt, INITIAL_CONNECT_MAX_ATTEMPTS
                        );
                        break;
                    }
                    Err(e) if attempt < INITIAL_CONNECT_MAX_ATTEMPTS => {
                        warn!(
                            "[client] failed to connect to node {} on attempt {}/{}: {}; retrying in {} ms",
                            node_id,
                            attempt,
                            INITIAL_CONNECT_MAX_ATTEMPTS,
                            e,
                            INITIAL_CONNECT_RETRY_DELAY.as_millis()
                        );
                        tokio::time::sleep(INITIAL_CONNECT_RETRY_DELAY).await;
                    }
                    Err(e) => {
                        error!(
                            "[client] failed to connect to node {} after {} attempts: {}",
                            node_id, INITIAL_CONNECT_MAX_ATTEMPTS, e
                        );
                    }
                }
            }
    ...
    fn record_transaction_sizes(&mut self, transactions: &[TestTransaction]) {
        let first_batch = self.stats.total_tx_generated == 0;
        let mut batch_bytes = 0u64;
        let mut measured_transactions = 0u64;
        ...
        for tx in transactions {
            match bincode::serialized_size(tx) {
                Ok(bytes) => {
                    ...
                    self.stats.record_tx_size(bytes);
                }
                Err(err) => warn!(
        ...
        if first_batch && measured_transactions > 0 {
            info!(
                "[client-stats] first generated batch: physical txs={}, total bincode tx bytes={}, avg tx bytes={:.1}",
                measured_transactions,
                batch_bytes,
                batch_bytes as f64 / measured_transactions as f64
            );
        }
    }
    ...
    pub async fn run_load_test(
        ...
        if node_num == 0 {
            error!("[load-test] NODE_NUM must be greater than zero");
            return;
        }

        let mut is_batch = false;
        let batch_size_constant: usize = env::var("CLIENT_BATCH_SIZE_CONSTANT")
            .unwrap_or_else(|_| "1".to_string())
            .parse()
            .expect("CLIENT_BATCH_SIZE_CONSTANT must be numeric");
        // let mut batch_size = std::cmp::max(100, config.target_tps / 5);
        let mut batch_size = match env::var("ORDERING_BATCH_SIZE") {
            Ok(raw) => match raw.trim().parse::<u32>() {
                Ok(value) if value > 0 => {
                    info!(
                        "[load-test] overriding batch size via ORDERING_BATCH_SIZE={}",
                        value
                    );
                    is_batch = true;
                    value
                ...
                Ok(_) => {
                    warn!(
                        "[load-test] ORDERING_BATCH_SIZE must be greater than zero; falling back to formula"
                    );
                    config.target_tps / (5 * node_num as u32)
                }
                Err(err) => {
                    warn!(
                        "[load-test] failed to parse ORDERING_BATCH_SIZE='{}': {}; using formula",
                        raw, err
                    );
                    config.target_tps / (5 * node_num as u32)
                }
        ...
        if !is_batch && batch_size_constant == 0 {
            error!("[load-test] CLIENT_BATCH_SIZE_CONSTANT must be greater than zero");
            return;
        }

        info!(
            "[load-test] dispatching one batch per interval in round-robin order across {} nodes; aggregate client rate does not scale with node count",
            node_num
        );

        while total_sent < max_transactions {
            let node_id = node_least_id + (batch_counter % node_num);
            let transactions: Vec<TestTransaction> = if is_batch {
                vec![self.tx_generator.generate_batched_tx(batch_size as usize)]
            } else {
                let remaining = max_transactions - total_sent;
                self.tx_generator
                    .generate_batch(batch_size_constant.min(remaining))
            };
...
fn setup_tracing_logger(mode: &str) {
    ...
    let requested_filter = env::var("CLIENT_LOG_LEVEL")
        .or_else(|_| env::var("RUST_LOG"))
        .unwrap_or_else(|_| "info".to_string());
    let (env_filter, effective_filter) = match EnvFilter::try_new(&requested_filter) {
        Ok(filter) => (filter, requested_filter),
        Err(err) => {
            eprintln!(
                "[logger] invalid CLIENT_LOG_LEVEL/RUST_LOG filter '{}': {}; using info",
                requested_filter, err
            );
    ...
    match result {
        Ok(_) => info!(
            "[logger] client logging initialized with filter '{}'",
            effective_filter
        ),
    ...
    tokio::select! {
        result = tokio::signal::ctrl_c() => {
            if let Err(err) = result {
                error!("[client] failed to listen for Ctrl+C: {}", err);
            }
        }
        _ = terminate.recv() => {}
    }
...
async fn print_final_latency_summary(latency_tracker: &Arc<Mutex<LatencyTracker>>) {
    info!("[latency] final summary before client exit");
...
async fn async_main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create channels
    let (_cmd_tx, mut cmd_rx) = tokio::sync::mpsc::unbounded_channel::<ClientCommand>();
    let (response_tx, mut response_rx) = tokio::sync::mpsc::unbounded_channel::<ResponseCommand>();

    let log_individual_latency = env_bool("CLIENT_LOG_INDIVIDUAL_LATENCY", true);
    let latency_warmup_secs = env_u64("CLIENT_LATENCY_WARMUP_SECS", 0);
    let latency_report_interval = env_u64(
        "CLIENT_LATENCY_REPORT_INTERVAL",
        DEFAULT_LATENCY_SUMMARY_INTERVAL as u64,
    )
    .max(1) as usize;
    info!(
        "[client] individual latency logging: {}; warm-up={} s; aggregate interval={} consensus samples",
        if log_individual_latency {
            "enabled"
        } else {
            "disabled (aggregate summaries only)"
        },
        latency_warmup_secs,
    ...
    // Launch latency tracker task
    {
        ...
        tokio::spawn(async move {
            ...
            let mut stats_reporter = StatsReporter::new(latency_report_interval);
    ...
    match mode.as_str() {
        "load_test" => {
            ...
            // Start the warm-up clock before the load generator. During this period
            // the consensus nodes keep advancing views/producing empty blocks, but
            // the client deliberately sends no transactions.
            latency_tracker.lock().await.start_measurement();

            if latency_warmup_secs > 0 {
                info!(
                    "[client] consensus-only warm-up for {} s; no transactions will be sent",
                    latency_warmup_secs
                );
                tokio::select! {
                    _ = tokio::time::sleep(Duration::from_secs(latency_warmup_secs)) => {
                        info!("[client] consensus-only warm-up complete; starting load test");
                    }
                    _ = shutdown_signal() => {
                        info!("[client] shutdown signal received during consensus-only warm-up");
                        print_final_latency_summary(&latency_tracker).await;
                        return Ok(());
                    }
                }
            }
            ...
            let interrupted = tokio::select! {
                ...
                _ = shutdown_signal() => {
                    info!("[client] shutdown signal received during load test");
                    true
                }
            };
            ...
            if !interrupted {
                tokio::select! {
                    ...
                    _ = shutdown_signal() => {
                        info!("[client] shutdown signal received while draining responses");
                    }
                }
            }
...
mod tests {
    ...
    #[test]
    fn stats_reporter_triggers_when_a_batch_crosses_the_threshold() {
        let mut reporter = StatsReporter::new(500);

        reporter.record_consensus_samples(498);
        assert!(!reporter.should_print_stats());

        reporter.record_consensus_samples(4);
        assert!(reporter.should_print_stats());
        assert!(!reporter.should_print_stats());

        reporter.record_consensus_samples(501);
        assert!(reporter.should_print_stats());
    }

    #[test]
    fn latency_tracker_excludes_warmup_samples() {
        let mut tracker = LatencyTracker::new(false, 20);
        tracker.start_measurement();
        let warmup_tx = TestTransaction {
            id: 1,
            from: "a".to_string(),
            to: "b".to_string(),
            amount: 1,
        ...
        tracker.record_send_time(&warmup_tx);
        assert!(!tracker.send_timestamps.contains_key(&warmup_tx.id));
        assert_eq!(tracker.excluded_warmup_transactions, 1);
    }
}
