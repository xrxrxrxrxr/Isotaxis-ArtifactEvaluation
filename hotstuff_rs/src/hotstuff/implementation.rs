...
use std::{env, sync::mpsc::Sender, time::SystemTime};
...
fn broadcast_phase_votes_enabled() -> bool {
    env::var("HS_BROADCAST_PHASE_VOTES")
        .ok()
        .is_some_and(|value| {
            matches!(
...
impl<N: Network> HotStuff<N> {
    ...
    fn on_receive_proposal<K: KVStore>(
        ...
        } = app.validate_block(validate_block_request)
        {
                ...
                || block_tree.highest_view_voted()?.unwrap() < self.view_info.view)
            {
                ...
                if broadcast_phase_votes_enabled() {
                    self.sender_handle
                        .broadcast::<HotStuffMessage>(phase_vote.clone().into());
                } else {
    ...
    fn on_receive_nudge<K: KVStore>(
            ...
            || block_tree.highest_view_voted()?.unwrap() < self.view_info.view)
        {
            ...
            if broadcast_phase_votes_enabled() {
                self.sender_handle
                    .broadcast::<HotStuffMessage>(vote.clone().into());
            } else {
