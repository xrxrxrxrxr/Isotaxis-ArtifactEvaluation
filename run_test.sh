...
if [ "$1" = "profile_node0" ] || [ "$1" = "profile" ]; then
...
elif [ "$1" = "attack" ] || [ "$1" = "adversary" ] || [ "$1" = "attack_only" ]; then
    CLIENT_MODE="load_test"
    ATTACK_MODE=true
    ...
    echo "Usage: $0 [interactive|load_test|perf_test|adversary|profile_node0] [--rebuild|--reuse]"
    exit 1
fi
...
ENV_CONFIG_FILE=".env"
if $ATTACK_MODE; then
    COMPOSE_CONFIG_FILE="docker-compose.attack.yml"
    ENV_CONFIG_FILE=".attack.env"
fi
if [ ! -f "$COMPOSE_CONFIG_FILE" ]; then
    echo "Please run this script from the directory containing $COMPOSE_CONFIG_FILE"
    exit 1
fi
...
if $ATTACK_MODE; then
    export COMPOSE_FILE="$COMPOSE_CONFIG_FILE"
    echo "Attack-only mode: using $COMPOSE_CONFIG_FILE with $ENV_CONFIG_FILE"
    echo "Attack mode: node1 floods ordering while participating in HotStuff"
fi
...
case $CLIENT_MODE in
    ...
    "load_test")
        if $ATTACK_MODE; then
            echo "Adversary load test started ($TARGET_TPS, node1 runs pure_attacker with HotStuff)"
        else
